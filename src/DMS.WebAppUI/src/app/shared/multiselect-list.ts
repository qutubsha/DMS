...
export interface IMultiSelectListOption {
    id: any;
    name: string;
}

export interface IMultiSelectListSettings {
    pullRight?: boolean;
...
export class MultiSelectSearchFilter {
    transform(options: Array<IMultiSelectListOption>, args: string): Array<IMultiSelectListOption> {
        return options.filter((option: IMultiSelectListOption) => option.name.toLowerCase().indexOf((args || '').toLowerCase()) > -1);
    }
}

@Component({
    selector: 'ss-multiselect-list',
    ...
    template: `
<div>
        <div class="btn-group multiple_select custom-multiselect-width">           
            <ul *ngIf="options.length > 0" class="dropdown-menu multi_ul position-custom" [class.pull-right]="settings.pullRight" 
            [style.max-height]="settings.maxHeight" style="display: block; height: auto; overflow-y: auto;">
                <li class="dropdown-item" style="margin: 0px 5px 5px 5px;" *ngIf="settings.enableSearch">
                    <div class="input-group input-group-sm multi-search">
                        ...
                        <input type="text" class="form-control not-common" placeholder="{{ texts.searchPlaceholder }}" 
                ...
                <li class="dropdown-item">
                    <a href="javascript:;" role="menuitem" tabindex="-1" (click)="checkAll(settings.showCheckAll)">
                         <input *ngIf="settings.checkedStyle == 'checkboxes'" id="checkAllCheckbox" type="checkbox" class="not-common" [checked]="settings.showCheckAll" />
                ...
                <li class="dropdown-item" *ngFor="let option of options | searchFilter:searchFilterText">
                    <a href="javascript:;" role="menuitem" tabindex="-1" (click)="setSelected($event, option)">
                        <input *ngIf="settings.checkedStyle == 'checkboxes'" type="checkbox" class="not-common" [checked]="isSelected(option)" />
                        <span *ngIf="settings.checkedStyle == 'glyphicon'" style="width: 16px;" 
...
export class MultiselectList implements OnInit, DoCheck, ControlValueAccessor {

    @Input() options: Array<IMultiSelectListOption>;
    @Input() settings: IMultiSelectListSettings;
    ...
    defaultSettings: IMultiSelectListSettings = {
        pullRight: false,
        enableSearch: true,
    ...
    isSelected(option: IMultiSelectListOption): boolean {
        return this.model && this.model.indexOf(option.id) > -1;
    }

    setSelected(event: Event, option: IMultiSelectListOption) {
        if (!this.model) {
            this.model = [];
        }
    ...
    updateTitle() {
        if (this.numSelected === 0) {
            ...
            let temptitle = this.options
                .filter((option: IMultiSelectListOption) => this.model && this.model.indexOf(option.id) > -1)
                .map((option: IMultiSelectListOption) => option.name)
                .join(', ');
